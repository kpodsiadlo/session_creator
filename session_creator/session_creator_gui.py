...
audio_table = sg.Table([[" " for col in range(3)] for row in range(10)],
                       headings=["Filename", "Bitrate",
                                 "Samplerate", "Channels"],
                       size=(54, 10), num_rows=10, col_widths=[22, 6, 9, 8],
                       auto_size_columns=False, key="_wav_data_table_",
                       justification='left')
...
layout = [
    ...
    [sg.Text('Distance between files (in multiples of length):'),
     sg.I(default_text='2', size=(3, None),
          key='_distance_'),  sg.Text(" "*22),
     sg.Text('Output file:'),
...
while True:
    ...
    if event == 'Check':
        ...
        wav_table_data = [[wav.filename, wav.bitrate, wav.samplerate,
                           wav.nchannels]
                          for wav in wave_data]
        window['_file_list_'].update(files_to_load)
        window['_wav_data_table_'].update(wav_table_data)
